                                    ...
                                    <div class="card-body">
                                        ...
                                        <div id="supplierOrderAdvancePaymentsSection" class="supplier-order-advance-field">
                                            ...
                                            <div class="supplier-order-new-advance-payment">
                                                ...
                                                <div class="form-row">
                                                    <div class="form-group col-md-4"><label>CUENTA BANCARIA ORIGEN</label><select name="advance_payments[0][company_bank_account_id]" id="supplier_order_new_advance_bank_account_id" class="form-control form-control-sm"><option value="">Seleccione cuenta</option>@foreach ($companyBankAccounts as $account)<option value="{{ $account->id }}" data-company-id="{{ $account->company_id }}" data-currency-id="{{ $account->currency_id }}">{{ $account->bank?->short_name ?: $account->bank?->description }} &middot; {{ $account->account_number }} &middot; {{ $account->currency?->code }}</option>@endforeach</select><span class="invalid-feedback"></span></div>
