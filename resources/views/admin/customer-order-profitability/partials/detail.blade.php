@php
    ...
    $costValue = function ($cost) use ($mode) {
        $total = (float) ($cost->total_amount ?: $cost->amount);
        $hasIgv = (bool) $cost->affects_igv && \App\Models\WarehouseEntryExpense::supportsIgv($cost->document_type);
        return $mode === 'without_igv' && $hasIgv ? (float) $cost->taxable_amount : $total;
    };
...
<div class="tab-content cop-tab-content">
    ...
    <div id="cop_costs" class="tab-pane fade">
        ...
        @foreach([['Flete / recojo / traslado con comprobante', $freightCosts, $freightValue, 'fa-truck', 'operating'], ['Otros gastos / sin comprobante', $otherCosts, $otherTotal, 'fa-receipt', 'net']] as [$title, $rows, $subtotal, $icon, $impact])
            ...
            <div class="table-responsive cop-inner-table"><table class="table table-hover mb-0"><thead><tr><th>Tipo</th><th>Responsable</th><th>Documento</th><th>Clasificación para rentabilidad</th><th>IGV</th><th>Fecha</th><th class="text-right">Importe considerado</th><th>Adjunto</th><th>Observación</th></tr></thead><tbody>@forelse($rows as $cost)<tr><td><strong>{{$costTypeLabel($cost)}}</strong></td><td>{{$cost->provider_name ?: '-'}}</td><td>{{collect([$cost->document_type,$cost->document_series,$cost->document_number])->filter()->join(' ') ?: 'Sin comprobante'}}</td><td><span class="cop-classification-badge {{$impact === 'operating' ? 'is-operational' : 'is-other'}}"><i class="fas {{$impact === 'operating' ? 'fa-check-circle' : 'fa-exclamation-circle'}}"></i>{{$impact === 'operating' ? 'Operativo con comprobante' : 'Otros gastos / sin comprobante'}}</span></td><td><span class="badge {{$cost->affects_igv ? 'badge-success' : 'badge-light'}}">{{$cost->affects_igv ? 'Afecto IGV' : 'Sin IGV'}}</span></td><td>{{$cost->document_date?->format('d/m/Y') ?: '-'}}</td><td class="text-right font-weight-bold">{{$money($costValue($cost))}}</td><td>{{$cost->documents->isNotEmpty() ? 'Sí' : 'No'}}</td><td>{{$cost->description ?: '-'}}</td></tr>@empty<tr><td colspan="9" class="cop-empty"><i class="fas {{$icon}}"></i>Sin registros en esta categoría.</td></tr>@endforelse</tbody></table></div></div>
        @endforeach
    </div>
    ...
    <div id="cop_taxes" class="tab-pane fade">
        ...
        <div class="table-responsive cop-inner-table"><table class="table table-hover mb-0"><thead><tr><th>Tipo de costo</th><th>Documento</th><th class="text-right">Importe total</th><th class="text-right">Base imponible</th><th class="text-right">IGV</th><th>Afecto IGV</th></tr></thead><tbody>@forelse($costs as $cost)<tr><td>{{$costTypeLabel($cost)}}</td><td>{{collect([$cost->document_type,$cost->document_series,$cost->document_number])->filter()->join(' ') ?: 'Sin comprobante'}}</td><td class="text-right">{{$money($cost->total_amount ?: $cost->amount)}}</td><td class="text-right">{{$money($cost->taxable_amount ?: $cost->amount)}}</td><td class="text-right font-weight-bold">{{$money($cost->igv_amount)}}</td><td><span class="badge {{$cost->affects_igv ? 'badge-success' : 'badge-light'}}">{{$cost->affects_igv ? 'Sí' : 'No'}}</span></td></tr>@empty<tr><td colspan="6" class="cop-empty"><i class="fas fa-receipt"></i>Sin costos vinculados.</td></tr>@endforelse</tbody></table></div>
