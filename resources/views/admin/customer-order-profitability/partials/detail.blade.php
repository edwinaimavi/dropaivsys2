...
<div class="tab-content cop-tab-content">
    ...
    <div id="cop_purchases" class="tab-pane fade">
        <div class="cop-section-heading"><div><span class="cop-eyebrow">Abastecimiento</span><h6>Compras vinculadas a la orden</h6><small>{{count($supplierOrderIds)}} orden(es) de compra a proveedor · importes normalizados a soles</small></div><div class="cop-section-total"><small>Compra considerada</small><strong>S/ {{$money($purchaseValue)}}</strong></div></div>
        <div class="table-responsive cop-inner-table"><table class="table table-hover"><thead><tr><th>OC proveedor</th><th>Proveedor / artículo</th><th>Moneda compra</th><th class="text-right">Total compra</th><th>Moneda pago</th><th class="text-right">TC</th><th class="text-right">Total en soles</th><th>Estado</th></tr></thead><tbody>@forelse($supplierItems as $item)@php($supplierStatus=\App\Models\CustomerPurchaseOrder::statusPresentation($item->order_status))<tr><td><strong>{{$item->order_code}}</strong></td><td><strong>{{$item->supplier_name}}</strong><small class="d-block text-muted">{{$item->billing_name_snapshot}} · Cant. {{$money($item->quantity)}}</small></td><td>{{$item->purchase_currency_code ?: '-'}}</td><td class="text-right">{{$item->purchase_currency_symbol}} {{$money($item->line_total)}}</td><td>{{$item->payment_currency_code ?: $item->purchase_currency_code ?: '-'}}</td><td class="text-right">{{(float)$item->pen_conversion_factor > 0 && $item->purchase_currency_code !== 'PEN' ? number_format((float)$item->pen_conversion_factor, 4) : '-'}}</td><td class="text-right font-weight-bold">S/ {{$money($item->line_total_pen)}}</td><td><span class="cop-status-pill {{$supplierStatus['class']}}"><i class="fas {{$supplierStatus['icon']}}"></i>{{$supplierStatus['label']}}</span></td></tr>@empty<tr><td colspan="8" class="cop-empty"><i class="fas fa-shopping-cart"></i>Sin compras vinculadas.</td></tr>@endforelse</tbody></table></div>
    </div>
    ...
    <div id="cop_costs" class="tab-pane fade">
        ...
        @foreach([['Flete / recojo / traslado con comprobante', $freightCosts, $freightValue, 'fa-truck', 'operating'], ['Otros gastos / sin comprobante', $otherCosts, $otherTotal, 'fa-receipt', 'net']] as [$title, $rows, $subtotal, $icon, $impact])
            ...
            <div class="table-responsive cop-inner-table"><table class="table table-hover mb-0"><thead><tr><th>Tipo</th><th>Responsable</th><th>Documento</th><th>Clasificación</th><th>IGV</th><th>Fecha</th><th class="text-right">Importe considerado</th><th>Adjuntos</th><th>Observación</th></tr></thead><tbody>@forelse($rows as $cost)@php($invoiceDocument=$expenseDocument($cost, 'invoice'))@php($paymentDocument=$expenseDocument($cost, 'payment_proof'))@php($officialDocument=\App\Models\WarehouseEntryExpense::isOfficialDocument($cost->document_type))<tr><td><strong>{{$costTypeLabel($cost)}}</strong></td><td>{{$cost->provider_name ?: '-'}}</td><td>{{collect([\App\Models\WarehouseEntryExpense::documentTypeLabel($cost->document_type),$cost->document_series,$cost->document_number])->filter()->join(' ')}}</td><td><span class="cop-classification-badge {{$officialDocument ? 'is-operational' : 'is-other'}}"><i class="fas {{$officialDocument ? 'fa-check-circle' : 'fa-exclamation-circle'}}"></i>{{$officialDocument ? 'Con comprobante oficial' : 'Otros gastos / sin comprobante'}}</span></td><td><span class="badge {{$costHasRecognizedIgv($cost) ? 'badge-success' : 'badge-light'}}">{{$costHasRecognizedIgv($cost) ? 'Afecto IGV' : 'Sin IGV'}}</span></td><td>{{$cost->document_date?->format('d/m/Y') ?: '-'}}</td><td class="text-right font-weight-bold">{{$money($costValue($cost))}}</td><td><small class="d-block">Comprobante: {{$invoiceDocument ? 'Sí' : 'No'}}</small><small class="d-block">Pago: {{$paymentDocument ? 'Sí' : 'No'}}</small></td><td>{{$cost->description ?: '-'}}</td></tr>@empty<tr><td colspan="9" class="cop-empty"><i class="fas {{$icon}}"></i>Sin registros en esta categoría.</td></tr>@endforelse</tbody></table></div></div>
        @endforeach
    </div>
