...
<div class="tab-content cop-tab-content">
    <div id="cop_summary" class="tab-pane fade show active">
        ...
        <div class="row cop-metrics-grid">
            @foreach([
                ['Venta considerada para utilidad', $saleProfitValue, 'sale', 'fa-coins', 'sale'],
                ['Compra considerada para utilidad', $purchaseProfitValue, 'purchase', 'fa-shopping-cart', 'purchases'],
    ...
    <div id="cop_purchases" class="tab-pane fade">
        @include('admin.customer-order-profitability.partials.supplier-purchases', compact('supplierItems', 'supplierOrderIds', 'purchaseValue', 'purchaseProfitValue', 'money'))
    </div>

    <div id="cop_costs" class="tab-pane fade">
        <div class="cop-section-heading"><div><span class="cop-eyebrow">Costos vinculados</span><h6>Detalle de gastos oficiales y no oficiales</h6><small>El total registrado es referencial. Los gastos oficiales afectos se descuentan siempre por su base sin IGV.</small></div><div class="cop-section-total"><small>Total registrado</small><strong>{{$money($linkedTotal)}}</strong></div></div>
        @foreach([['Costos con comprobante oficial', $freightCosts, $freightValue, 'fa-file-invoice', 'operating'], ['Otros gastos / sin comprobante', $otherCosts, $otherTotal, 'fa-receipt', 'net']] as [$title, $rows, $subtotal, $icon, $impact])
            <div class="cop-cost-block"><div class="cop-cost-block-header"><div><span class="cop-cost-icon"><i class="fas {{$icon}}"></i></span><div><h6>{{$title}}</h6><small>{{$impact === 'operating' ? 'Monto usado en utilidad operativa; sin IGV cuando el modo y el documento correspondan.' : 'Se descuenta por el monto completo después del impuesto a la renta.'}}</small></div></div><span class="cop-cost-subtotal">Usado en cálculo <strong>{{$money($subtotal)}}</strong></span></div>
    ...
    <div id="cop_taxes" class="tab-pane fade">
        ...
        <div class="table-responsive cop-inner-table mb-4"><table class="table table-hover mb-0"><thead><tr><th>Concepto</th><th class="text-right">Total registrado</th><th class="text-right">Base imponible</th><th class="text-right">IGV</th><th class="text-right">Usado en utilidad</th></tr></thead><tbody><tr><td><strong>Venta al cliente</strong></td><td class="text-right">{{$money($saleTotal)}}</td><td class="text-right">{{$money($saleBase)}}</td><td class="text-right">{{$money($saleIgv)}}</td><td class="text-right font-weight-bold">{{$money($saleProfitValue)}}</td></tr><tr><td><strong>Compras a proveedor / Almacén</strong></td><td class="text-right">{{$money($purchaseTotal)}}</td><td class="text-right">{{$money($purchaseBase)}}</td><td class="text-right">{{$money($purchaseIgv)}}</td><td class="text-right font-weight-bold">{{$money($purchaseProfitValue)}}</td></tr><tr><td><strong>Costos vinculados</strong></td><td class="text-right">{{$money($linkedGrossTotal)}}</td><td class="text-right">{{$money($linkedBase)}}</td><td class="text-right">{{$money($linkedIgv)}}</td><td class="text-right font-weight-bold">{{$money($linkedProfitValue)}}</td></tr></tbody></table></div>
        <div class="cop-section-heading mt-4"><div><span class="cop-eyebrow">Detalle de costos</span><h6>IGV de costos vinculados</h6><small>El IGV solo se reconoce en factura o boleta cuando el costo fue marcado como afecto.</small></div><div class="cop-section-total"><small>IGV vinculado</small><strong>{{$money($igvLinkedCosts)}}</strong></div></div>
    ...
    <div id="cop_profit" class="tab-pane fade">
        ...
        <div class="cop-statement"><div><span>Venta usada en utilidad</span><strong>{{$money($saleProfitValue)}}</strong></div><div><span>- Compra usada en utilidad</span><strong>{{$money($purchaseProfitValue)}}</strong></div><div class="total"><span>= Utilidad bruta</span><strong>{{$money($gross)}}</strong></div><div><span>- Costos con comprobante oficial</span><strong>{{$money($freightValue)}}</strong></div><div class="total"><span>= Utilidad operativa</span><strong>{{$money($operating)}}</strong></div><div><span>- Impuesto renta estimado</span><strong>{{$money($incomeTax)}}</strong></div><div><span>- Otros gastos / sin comprobante</span><strong>{{$money($otherTotal)}}</strong></div><div class="total cop-final-total"><span>= Utilidad neta</span><strong>{{$money($net)}}</strong></div></div>
    </div>
</div>
