...
<div id="pettyCashApp"
    ...
    data-can-receipt-exchange-update="{{ auth()->user()->can('admin.petty-cash.receipt-exchanges.update') ? 1 : 0 }}"
    data-can-receipt-exchange-reverse="{{ auth()->user()->can('admin.petty-cash.receipt-exchanges.reverse') ? 1 : 0 }}"
    ...
    <div class="card border-0 shadow-lg rounded-xl petty-cash-table-card">
            ...
            <div class="table-responsive petty-cash-table-responsive">
                <table id="tablePettyCash" class="table table-hover align-middle text-center w-100">
                    <thead class="bg-light">
                        <tr>
                            <th class="petty-responsive-control" aria-label="Ver más"></th><th>#</th><th>ID</th><th>CÓDIGO</th><th>EMPRESA</th><th>PERIODO</th>
...
@endif
@if(auth()->user()->canAny(['admin.petty-cash.receipt-exchanges.update', 'admin.petty-cash.receipt-exchanges.reverse']))
@include('admin.petty-cash.partials.receiptExchangeEditModal')
@endif
@canany(['admin.petty-cash.expenses.approve', 'admin.petty-cash.expenses.observe'])
...
<style>
    ...
    .petty-approval-expense-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.petty-approval-expense-grid>div,.petty-approval-documents{padding:10px;border:1px solid #e3e9e6;border-radius:10px;background:#fff}.petty-approval-expense-grid small,.petty-approval-documents>small{display:block;color:#84908c;font-size:.62rem;font-weight:800;letter-spacing:.05em}.petty-approval-expense-grid strong{display:block;margin-top:3px;color:#293b36;font-size:.8rem}.petty-approval-documents a{display:inline-flex;align-items:center;margin:7px 6px 0 0;padding:6px 9px;border-radius:8px;background:#edf6f2;color:#237c5c;font-size:.72rem}.expense-admin-summary-note{padding:10px 12px;border:1px solid #dce7e2;border-left:3px solid #5f8f7c;border-radius:10px;background:#f7faf9}.expense-admin-summary-note span{display:block;color:#648074;font-size:.61rem;font-weight:900;letter-spacing:.065em}.expense-admin-summary-note p{margin:4px 0 0;color:#34463f;font-size:.78rem;line-height:1.45;white-space:pre-wrap;overflow-wrap:anywhere}.petty-approval-badge{display:inline-block;padding:4px 7px;border-radius:20px;font-size:.58rem;font-weight:800}.petty-approval-badge.is-pending{background:#f7edd3;color:#795a18}.petty-approval-badge.is-approved{background:#dff2e8;color:#216c4f}.petty-approval-badge.is-rejected{background:#f8e4e4;color:#9a3d3d}.petty-approval-badge.is-cancelled{background:#ecefee;color:#66716d}.petty-approval-trace{display:block;margin-top:4px;color:#8a9692;font-size:.57rem;line-height:1.3}
    .petty-review-history{padding:11px;border:1px solid #e3e9e6;border-radius:10px;background:#f8fbfa}.petty-review-history>small{display:block;color:#84908c;font-size:.62rem;font-weight:800;letter-spacing:.05em}.petty-review-history article{display:flex;gap:9px;padding:9px 0;border-bottom:1px solid #e7eeeb}.petty-review-history article:last-child{border-bottom:0}.petty-review-history article>i{display:flex;width:28px;height:28px;align-items:center;justify-content:center;border-radius:50%;background:#e5f3ed;color:#237c5c}.petty-review-history article strong,.petty-review-history article small{display:block}.petty-review-history article small{color:#7b8983;font-size:.68rem}.petty-review-history article p{margin:3px 0 0;font-size:.72rem}.petty-review-actions .selectExpenseReviewAction.active{box-shadow:0 0 0 3px rgba(32,118,92,.18);transform:translateY(-1px)}
    .petty-lifted-observation{padding:10px 12px;border:1px solid #cddff0;border-left:3px solid #4b82b2;border-radius:10px;background:#f4f8fc}.petty-lifted-observation-heading{display:flex;align-items:center;justify-content:space-between;gap:8px}.petty-lifted-observation-heading>span{color:#456f94;font-size:.61rem;font-weight:900;letter-spacing:.065em}.petty-lifted-observation-heading .btn{padding:0;color:#4b7598;font-size:.62rem}.petty-lifted-observation p{margin:5px 0;color:#344b5f;font-size:.76rem;line-height:1.45;white-space:pre-wrap}.petty-lifted-observation>small{color:#728596;font-size:.61rem}
    ...
    #tablePettyCash th.petty-responsive-control,#tablePettyCash td.petty-responsive-control{width:34px;min-width:34px;padding-left:8px!important;padding-right:8px!important}
    #tablePettyCash.collapsed tbody td.dtr-control:before{display:inline-flex!important;width:20px;height:20px;align-items:center;justify-content:center;border:0;border-radius:50%;background:#e3f3ec;color:#207557;box-shadow:none;font-family:"Font Awesome 5 Free";font-size:9px;font-weight:900;line-height:1;content:"\f067"}
    #tablePettyCash.collapsed tbody tr.parent td.dtr-control:before{background:#f8e8e9;color:#b43d48;content:"\f068"}
    .petty-cash-table-card,
    .petty-cash-table-card>.card-body,
    .petty-cash-table-card #tablePettyCash_wrapper,
    .petty-cash-table-card #tablePettyCash_wrapper>.row,
    .petty-cash-table-card .petty-cash-table-responsive{overflow:visible!important}
    .petty-main-actions{position:relative;display:inline-flex;gap:5px;align-items:center;white-space:nowrap}
    .petty-cash-actions-menu{z-index:1049;min-width:210px;max-width:calc(100vw - 24px);max-height:calc(100vh - 24px);overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain}
    .table-responsive.petty-dropdown-is-open{overflow:visible!important}
    ...
    .petty-settlement-document{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:0 0 6px;padding:8px 9px;border:1px solid #e4ebe8;border-radius:9px;background:#fff;list-style:none}.petty-settlement-document.is-reversed{border-color:#f3cccc;background:#fff8f8}.petty-settlement-document-main{min-width:0}.petty-settlement-document-main strong{white-space:normal}.petty-settlement-document-meta{margin-top:2px;color:#7a8883!important}.petty-settlement-document-actions{flex:0 0 auto}.petty-settlement-document-actions .dropdown-menu{min-width:155px;padding:5px;border:0;border-radius:10px;box-shadow:0 12px 28px rgba(15,23,42,.16)}.petty-settlement-document-actions .dropdown-item{border-radius:7px;font-size:.7rem;font-weight:700}.petty-settlement-reversed-badge{display:inline-block!important;margin-top:4px;padding:3px 7px;border-radius:999px;background:#fee2e2;color:#b42318!important;font-size:.55rem!important;font-weight:900!important}.petty-settlement-edit-modal .modal-content{border-radius:16px;overflow:hidden}.petty-settlement-edit-modal .modal-header{align-items:center;border:0;background:linear-gradient(135deg,#edf8f3,#f8fbfa)}.petty-settlement-edit-modal .modal-header small{color:#6f817a;font-size:.58rem;font-weight:900;letter-spacing:.08em}.petty-settlement-edit-modal .modal-title{color:#174c3b;font-size:1.05rem;font-weight:800}.petty-settlement-edit-modal .modal-body{background:#fbfcfc}.petty-settlement-edit-modal label{color:#5d6d67;font-size:.68rem;font-weight:800}.petty-settlement-edit-modal .form-control{border-color:#dce6e2;border-radius:8px;box-shadow:none}.petty-settlement-file-box{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px;border:1px dashed #b8ccc4;border-radius:10px;background:#f4faf7}.petty-settlement-file-box strong,.petty-settlement-file-box small{display:block}.petty-settlement-file-box small{color:#7c8c86;font-size:.62rem}
    ...
    .petty-main-actions{display:inline-flex;align-items:center;gap:5px}.petty-main-actions>.btn,.petty-actions-trigger{height:31px;border-radius:8px;font-size:.68rem;font-weight:800}.petty-actions-trigger:after{display:none}.petty-actions-menu{min-width:220px;padding:7px;border:1px solid #e0e7e4;border-radius:12px;box-shadow:0 14px 35px rgba(15,23,42,.16)}.petty-actions-menu .dropdown-header{padding:5px 9px;color:#96a09c;font-size:.52rem;font-weight:900;letter-spacing:.08em;text-transform:uppercase}.petty-actions-menu .dropdown-item{display:flex;gap:7px;padding:7px 9px;align-items:center;border:0;border-radius:7px;background:transparent;color:#42534d;font-size:.67rem}.petty-actions-menu .dropdown-item i{width:20px;margin-right:0;text-align:center}.petty-actions-menu .dropdown-item:hover{background:#f0f7f4;color:#205f49}.petty-actions-menu .dropdown-item:disabled{color:#adb5b2;background:transparent}.petty-actions-menu .dropdown-divider{margin:5px 0;border-color:#edf1ef}
    .petty-cash-detail-modal .modal-content{display:flex;max-height:calc(100vh - 28px);flex-direction:column;overflow:hidden}.petty-cash-detail-modal .petty-detail-header{flex:0 0 auto}.petty-detail-tabs{flex:0 0 auto;padding:7px 12px;border-bottom:1px solid #e1e8e5;background:#fff}.petty-detail-tabs .nav{display:flex;gap:5px;flex-wrap:nowrap}.petty-detail-tabs .nav-link{display:flex;min-width:0;gap:6px;padding:7px 11px;align-items:center;border-radius:9px;color:#6e7e78;font-size:.65rem;font-weight:800;white-space:nowrap}.petty-detail-tabs .nav-link i{margin:0;color:#8ba097}.petty-detail-tabs .petty-tab-count{display:inline-flex;min-width:18px;height:18px;margin-left:2px;padding:0 5px;align-items:center;justify-content:center;border-radius:999px;background:#edf2f0;color:#71807b;font-size:.52rem}.petty-detail-tabs .nav-link.active{background:#e7f4ee;color:#1f6f51}.petty-detail-tabs .nav-link.active i{color:#287f5f}.petty-detail-tabs .nav-link.active .petty-tab-count{background:#cae7da;color:#1d6a4e}.petty-cash-detail-modal .petty-detail-body{min-height:0;overflow-y:auto}.petty-cash-detail-modal .tab-pane{min-height:365px}.petty-tab-toolbar{display:flex;align-items:center;justify-content:space-between;margin-bottom:9px}.petty-tab-toolbar h6{margin:0;color:#2d423a;font-size:.82rem;font-weight:900}.petty-tab-toolbar small{display:block;margin-top:2px;color:#929d99;font-size:.61rem}.petty-tab-toolbar>div:last-child{display:flex;gap:5px;align-items:center}.petty-tab-table{max-height:390px;overflow:auto}.petty-audit-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.petty-audit-grid>div{padding:10px;border:1px solid #e1e9e5;border-radius:10px;background:#fafcfb}.petty-audit-grid small,.petty-audit-grid strong,.petty-audit-grid span{display:block}.petty-audit-grid small{color:#8d9995;font-size:.55rem;font-weight:900;text-transform:uppercase}.petty-audit-grid strong{margin-top:3px;color:#344a42;font-size:.71rem}.petty-audit-grid span{margin-top:2px;color:#89958f;font-size:.58rem}
    ...
    .petty-approval-badge.is-lifted{margin-left:4px;background:#e9e5ff;color:#5843a5}
    ...
    .petty-correction-card{border-color:#d7cff8;background:#fbfaff}
    .petty-observation-timeline{position:relative;padding-left:32px}
    .petty-observation-timeline:before{position:absolute;top:8px;bottom:8px;left:13px;width:2px;background:#e4e1ed;content:""}
    .petty-observation-timeline-item{position:relative;margin-bottom:14px}
    .petty-observation-timeline-marker{position:absolute;top:4px;left:-32px;z-index:1;display:flex;width:28px;height:28px;align-items:center;justify-content:center;border:3px solid #fff;border-radius:50%;background:#e9a52c;color:#fff;font-size:.65rem;box-shadow:0 0 0 1px #e8d4aa}
    .petty-observation-timeline-item.is-resolved .petty-observation-timeline-marker{background:#6654b8;box-shadow:0 0 0 1px #cfc7ef}
    .petty-observation-timeline-card{padding:13px 14px;border:1px solid #e8e4ef;border-radius:12px;background:#fff;box-shadow:0 3px 12px rgba(51,39,83,.05)}
    .petty-observation-timeline-card>small,.petty-observation-timeline-card section>small{color:#9a650c;font-size:.59rem;font-weight:900;letter-spacing:.08em}
    .petty-observation-timeline-card h6{margin:3px 0 7px;color:#453b2c;font-size:.8rem;font-weight:800}
    .petty-observation-timeline-card p{margin-bottom:8px;color:#55505f;font-size:.76rem;line-height:1.5;white-space:pre-wrap}
    .petty-observation-timeline-card>div,.petty-observation-timeline-card section>div{color:#817a8a;font-size:.65rem}
    .petty-observation-timeline-card section{margin-top:12px;padding:11px 12px;border-radius:9px;background:#f2efff}
    .petty-observation-timeline-card section>small{color:#5843a5}
    .petty-observation-timeline-card section p{margin:4px 0 7px;color:#48405f}
    .petty-observation-open-label{display:inline-block;margin-top:10px;padding:4px 8px;border-radius:12px;background:#fff0cf;color:#8b5907;font-size:.61rem;font-weight:800}
    @media(max-width:575px){.petty-observation-timeline{padding-left:28px}.petty-observation-timeline-marker{left:-28px;width:24px;height:24px}.petty-observation-timeline-card{padding:11px}}
    .petty-expense-detail-modal .modal-content{max-height:calc(100vh - 32px);overflow:hidden;border-radius:18px;background:#f5f8f7;box-shadow:0 26px 70px rgba(15,23,42,.22)}
    .petty-expense-detail-loading{display:flex;min-height:180px;align-items:center;justify-content:center;gap:9px;color:#708079;font-size:.78rem;font-weight:700}
    .petty-expense-detail-tabs-wrap{margin-bottom:10px;overflow-x:auto;overflow-y:hidden;scrollbar-width:thin;scrollbar-color:#c7d5cf transparent}
    .petty-expense-detail-tabs{display:flex;min-width:max-content;gap:5px;padding:4px;border:1px solid #e0e8e4;border-radius:12px;background:#fff}
    .petty-expense-detail-tabs .nav-link{display:flex;min-height:35px;align-items:center;gap:6px;padding:7px 11px;border-radius:9px;color:#697973;font-size:.68rem;font-weight:800;white-space:nowrap;transition:.18s}
    .petty-expense-detail-tabs .nav-link i{font-size:.62rem}.petty-expense-detail-tabs .nav-link b{display:inline-flex;min-width:18px;height:18px;align-items:center;justify-content:center;padding:0 5px;border-radius:9px;background:#edf2f0;color:#6e7d77;font-size:.55rem}
    .petty-expense-detail-tabs .nav-link:hover{background:#f2f7f5;color:#356e59}.petty-expense-detail-tabs .nav-link.active{background:linear-gradient(135deg,#e3f3ec,#d8ece4);color:#236d52;box-shadow:inset 0 0 0 1px rgba(38,116,87,.08)}
    .petty-expense-detail-tabs .nav-link.active b{background:#fff;color:#28765a}.petty-expense-detail-tab-content{min-height:260px}
    .petty-expense-detail-section{margin-bottom:11px;padding:13px;border:1px solid #e1e8e5;border-radius:13px;background:#fff;box-shadow:0 4px 14px rgba(15,23,42,.035)}
    .petty-expense-detail-heading{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
    .petty-expense-detail-heading small{display:block;color:#27805f;font-size:.57rem;font-weight:900;letter-spacing:.095em}
    .petty-expense-detail-heading h6{margin:2px 0 0;color:#2d3e38;font-size:.86rem;font-weight:800}
    .petty-expense-detail-heading>i{display:flex;width:28px;height:28px;align-items:center;justify-content:center;border-radius:8px;background:#edf4f1;color:#568070;font-size:.68rem}
    .petty-expense-detail-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px}
    .petty-expense-detail-grid>div{min-width:0;padding:8px 9px;border:1px solid #e7ecea;border-radius:9px;background:#fafcfb}
    .petty-expense-detail-grid small,.petty-expense-detail-grid strong{display:block}
    .petty-expense-detail-grid small{color:#83908b;font-size:.55rem;font-weight:800;letter-spacing:.035em;text-transform:uppercase}
    .petty-expense-detail-grid strong{margin-top:3px;overflow-wrap:anywhere;color:#30413b;font-size:.72rem;line-height:1.3}
    .petty-expense-detail-documents{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
    .petty-expense-detail-documents>article{display:flex;min-width:0;align-items:center;gap:9px;padding:8px;border:1px solid #e3e9e6;border-radius:10px;background:#fafcfb}
    .petty-expense-document-preview,.petty-expense-document-pdf{display:flex;width:42px;height:42px;flex:0 0 42px;align-items:center;justify-content:center;overflow:hidden;border-radius:8px;background:#edf3f0;color:#b44545;font-size:1.15rem}
    .petty-expense-document-preview img{width:100%;height:100%;object-fit:cover}
    .petty-expense-detail-documents article>div:nth-child(2){min-width:0;flex:1}
    .petty-expense-detail-documents strong,.petty-expense-detail-documents small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .petty-expense-detail-documents strong{color:#35463f;font-size:.69rem}.petty-expense-detail-documents small{margin-top:2px;color:#8a9691;font-size:.56rem}
    .petty-expense-document-actions{display:flex;gap:4px}.petty-expense-document-actions a{display:flex;width:29px;height:29px;align-items:center;justify-content:center;border-radius:7px;background:#e8f3ee;color:#27775a;font-size:.64rem}.petty-expense-document-actions span{display:none}
    .petty-expense-detail-empty{padding:12px;border:1px dashed #d7dfdc;border-radius:9px;background:#fafcfb;color:#7b8984;font-size:.7rem;text-align:center}
    .petty-expense-admin-timeline{position:relative;padding-left:34px}.petty-expense-admin-timeline:before{position:absolute;top:6px;bottom:6px;left:13px;width:2px;background:#e1e7e4;content:""}
    .petty-expense-timeline-item{position:relative;margin-bottom:9px}.petty-expense-timeline-item:last-child{margin-bottom:0}
    ...
    @media(max-width:767px){.petty-expense-detail-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.petty-expense-detail-documents{grid-template-columns:1fr}}
    @media(max-width:575px){.petty-expense-approval-grid{grid-template-columns:1fr}.petty-expense-admin-card.is-exchange{grid-column:auto}.petty-expense-admin-card>div{grid-template-columns:1fr}}
    @media(max-width:480px){.petty-expense-detail-grid{grid-template-columns:1fr}.petty-expense-timeline-item header{align-items:flex-start;flex-direction:column;gap:1px}}
</style>
@endpush
